            ...
            <div className="relative top-10 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white print:shadow-none print:border-none print:w-full print:max-w-none print:top-0">
                ...
                <div className="print-content text-sm">
                    ...
                    <div className="flex justify-between mb-4">
                        <div>
                            <span className="font-bold">Date:</span> {shift?.start ? format(new Date(shift.start), 'MM/dd/yyyy') : ''}
                        </div>
                        <div>
                            <span className="font-bold">Shift:</span> {shift?.start && shift?.end ?
                                `${format(new Date(shift.start), 'h:mm a')} - ${format(new Date(shift.end), 'h:mm a')}` : ''}
                        </div>
                        <div>
                            <span className="font-bold">Role:</span> {shift?.role_id === 4 ? 'Maintenance' : 'Cashier'}
                        </div>
                    </div>
                    ...
                    {/* Logic: If Cashier (Role 3,7,8), enable pagination. Else (Maint), keep single list. */
                        (shift?.role_id === 4) ? (
                            ...
                            <div>
                                ...
                                {(() => {
                                    // Group candidates
                                    // Page 1: Priority PT + Full Time (Standard < 40h)
                                    const p1_data = candidates.filter(c =>
                                        c.section.includes("Priority") ||
                                        c.section === "Full Time Cashiers"
                                    );

                                    // Page 2: Full Time (OT / > 40h)
                                    const p2_data = candidates.filter(c =>
                                        c.section === "Full Time Cashiers (OT)"
                                    );

                                    // Page 3: Relaxed PT / Extended
                                    const p3_data = candidates.filter(c =>
                                        c.section.includes("Relaxed") ||
                                        c.section.includes("Extended")
                                    );

                                    const pages = [
                                        { title: "Page 1: Primary Call Sheet", data: p1_data },
                                        { title: "Page 2: Full Time Overtime", data: p2_data },
                                        { title: "Page 3: Extended / Relaxed", data: p3_data }
                                    ];
